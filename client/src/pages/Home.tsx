...
const ASSETS = {
  ...
  gallery: {
    manuscript: "/manus-storage/raad-gallery-manuscript_27b27071.jpg",
    desk: "/manus-storage/raad-gallery-research-desk_426f7bc8.jpg",
    hall: "/manus-storage/raad-gallery-lecture-hall_5cc40f25.jpg",
    library: "/manus-storage/raad-gallery-library_10b1173b.jpg",
  },
};

type Locale = "ar" | "en";
const navIds = ["overview", "education", "experience", "teaching", "research", "gallery", "service", "honors", "contact"] as const;
...
export default function Home() {
  ...
  const galleryItems = galleryData[language];
  ...
  const navItems = [
    { id: "overview", label: copy.nav.overview }, { id: "education", label: copy.nav.education }, { id: "experience", label: copy.nav.experience }, { id: "teaching", label: copy.nav.teaching }, { id: "research", label: copy.nav.research }, { id: "gallery", label: isEnglish ? "Gallery" : "المعرض" }, { id: "service", label: isEnglish ? "Service" : "اللجان" }, { id: "honors", label: copy.nav.honors }, { id: "contact", label: copy.nav.contact },
  ];
  ...
  return (
    <div className={`site-shell ${isEnglish ? "is-english" : ""} ${darkMode ? "is-dark" : ""}`} dir={isEnglish ? "ltr" : "rtl"}>
      ...
      <main>
        ...
        <section id="gallery" className="gallery-section"><div className="container"><div className="gallery-heading"><SectionHeading eyebrow={isEnglish ? "Archival gallery" : "المعرض الأرشيفي"} title={isEnglish ? "A visual reading room" : "غرفة قراءة بصرية"} copy={isEnglish ? "Atmospheric images for the research, teaching, and manuscript dimensions of the dossier." : "صور أجواء بحثية وتعليمية ومخطوطية تكمل قراءة الملف الأكاديمي."} number="07" /><span className="gallery-note"><Images size={18} />{isEnglish ? "Visual archive" : "أرشيف بصري"}</span></div><div className="gallery-grid">{galleryItems.map((item, index) => <figure className={`gallery-card gallery-card-${index + 1}`} key={item.image}><img src={item.image} alt={item.title} loading="lazy" /><figcaption><span>{item.tag}</span><strong>{item.title}</strong><p>{item.description}</p></figcaption></figure>)}</div><p className="gallery-disclaimer">{isEnglish ? "These are editorial archival atmospheres, not personal photographs of the professor. Official portraits and event photographs can be added when supplied." : "هذه صور أجواء أرشيفية تحريرية وليست صوراً شخصية للأستاذ الدكتور. يمكن إضافة الصور الرسمية وصور المؤتمرات عند تزويدنا بها."}</p></div></section>
        <section id="service" className="section-shell service-section"><div className="container"><SectionHeading eyebrow={copy.service.eyebrow} title={copy.service.title} copy={copy.service.copy} number="08" /><div className="service-layout"><div className="service-list">{copy.service.items.map((item, index) => <article className="service-item" key={item}><span>{String(index + 1).padStart(2, "0")}</span><p>{item}</p><Check size={15} /></article>)}</div><div className="skills-card"><h3>{copy.service.skillsTitle}</h3><div>{copy.service.skills.map((skill) => <p key={skill}><Sparkles size={14} />{skill}</p>)}</div></div></div></div></section>
        <section id="honors" className="section-shell honors-section"><div className="container honors-grid"><div><SectionHeading eyebrow={copy.honors.eyebrow} title={copy.honors.title} copy={copy.honors.copy} number="09" /><div className="honors-quote"><Quote size={22} /><p>{isEnglish ? "What we leave in others is what gives achievement its complete meaning." : "وحده ما نتركه في الآخرين يمنح الإنجاز معناه الكامل."}</p></div></div><div className="awards-list">{copy.honors.items.map((award, index) => <article className="award-item" key={award}><div className="award-number">{String(index + 1).padStart(2, "0")}</div><div><p>{award}</p><span><Award size={14} /> {isEnglish ? "Documented in the academic dossier" : "تكريم موثق في السيرة الذاتية"}</span></div></article>)}</div></div></section>
